<div class="p-6">
    <div class="flex items-center justify-between mb-6">
        <div>
            <flux:heading size="xl" class="text-zinc-900">{{ __('Manajemen Guru') }}</flux:heading>
            <flux:subheading class="text-zinc-600">{{ __('Kelola data guru, wali kelas, dan BK') }}</flux:subheading>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-zinc-50 border-b border-zinc-200">
                        ...
                        <th class="text-left p-4 font-semibold text-zinc-700">{{ __('NIP') }}</th>
                        <th class="text-left p-4 font-semibold text-zinc-700">{{ __('No HP') }}</th>
                        <th class="text-left p-4 font-semibold text-zinc-700">{{ __('Status') }}</th>
                        <th class="text-left p-4 font-semibold text-zinc-700">{{ __('Wali Kelas') }}</th>
                        <th class="text-left p-4 font-semibold text-zinc-700">{{ __('BK') }}</th>
                ...
                <tbody class="divide-y divide-zinc-200">
                    @forelse($semuaGuru as $guru)
                        <tr class="hover:bg-zinc-50 cursor-pointer transition"
                            ...
                            <td class="p-4 font-medium text-zinc-900">{{ $guru->nama }}</td>
                            <td class="p-4 text-zinc-600">{{ $guru->nip }}</td>
                            <td class="p-4 text-zinc-600">{{ $guru->no_hp ?? '-' }}</td>
                            <td class="p-4">
                                <flux:badge :color="$guru->status_aktif === 'Aktif' ? 'green' : 'zinc'" size="sm">
                                    {{ $guru->status_aktif }}
                                </flux:badge>
                            </td>
                            <td class="p-4 text-zinc-600">{{ $guru->wali_kelas_nama }}</td>
                            <td class="p-4 text-zinc-600">{{ $guru->bk_tingkat_list }}</td>
                            <td class="p-4 text-right">
                                <div class="flex justify-end gap-1" wire:click.stop>
                                    <flux:button size="xs" wire:click="openEdit({{ $guru->id }})" icon="pencil-square" variant="ghost" />
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="p-12 text-center">
                                ...
                                <flux:heading size="base" class="text-zinc-500 mb-1">{{ __('Belum ada data guru') }}</flux:heading>
                                <flux:text class="text-zinc-400">{{ __('Klik tombol "Tambah Guru" untuk memulai') }}</flux:text>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="space-y-4">
            ...
            <div class="flex border-b border-zinc-200">
                <button wire:click="setDetailTab('data')"
                    class="px-4 py-2.5 text-sm font-medium border-b-2 transition {{ $detailTab === 'data' ? 'border-brand-600 text-brand-600' : 'border-transparent text-zinc-500 hover:text-zinc-700' }}">
                    {{ __('Data Guru') }}
                </button>
                <button wire:click="setDetailTab('walikelas')"
                    class="px-4 py-2.5 text-sm font-medium border-b-2 transition {{ $detailTab === 'walikelas' ? 'border-brand-600 text-brand-600' : 'border-transparent text-zinc-500 hover:text-zinc-700' }}">
                    {{ __('Wali Kelas') }}
                </button>
                <button wire:click="setDetailTab('bk')"
                    class="px-4 py-2.5 text-sm font-medium border-b-2 transition {{ $detailTab === 'bk' ? 'border-brand-600 text-brand-600' : 'border-transparent text-zinc-500 hover:text-zinc-700' }}">
                    {{ __('BK') }}
                </button>
            </div>
            ...
            @if($detailTab === 'data' && $g)
                <div class="grid grid-cols-2 gap-4">
                    <div><flux:text class="text-zinc-600 font-medium text-sm">{{ __('Nama') }}</flux:text><flux:text class="text-zinc-900">{{ $g->nama }}</flux:text></div>
                    <div><flux:text class="text-zinc-600 font-medium text-sm">{{ __('NIP') }}</flux:text><flux:text class="text-zinc-900">{{ $g->nip }}</flux:text></div>
                    <div><flux:text class="text-zinc-600 font-medium text-sm">{{ __('No HP') }}</flux:text><flux:text class="text-zinc-900">{{ $g->no_hp ?? '-' }}</flux:text></div>
            ...
            @elseif($detailTab === 'bk')
                <form wire:submit="saveBk" class="space-y-4">
                    <flux:text class="text-zinc-700">{{ __('Pilih tingkat yang dibimbing:') }}</flux:text>
                    <div class="space-y-2">
                        @foreach([10, 11, 12] as $t)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <flux:checkbox wire:model="bk_tingkat" value="{{ $t }}" />
                                <span class="text-zinc-700">{{ __('Tingkat') }} {{ $t }}</span>
                            </label>
                        @endforeach
                    </div>
                    <div class="flex justify-end">
