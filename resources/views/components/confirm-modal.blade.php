<div
    x-data="{ open: false, title: '', message: '', formId: '', loading: false, step: 1, secondMessage: '' }"
    x-on:open-confirm-modal.window="
        open = true; loading = false; step = 1;
        title = $event.detail.title || '';
        message = $event.detail.message || '';
        secondMessage = $event.detail.secondMessage || '';
            ...
            <div class="mt-6 flex justify-end gap-3">
                ...
                <template x-if="secondMessage">
                    <button @click="step = step === 1 ? 2 : (loading = true, document.getElementById(formId).submit())"
                        ...
                        <span x-text="step === 1 ? 'Saya Mengerti' : 'Hapus Semua'"></span>
                    </button>
                </template>
            </div>
        </div>
