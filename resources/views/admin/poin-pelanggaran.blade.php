    ...
    <div class="table-container">
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr><td class="table-cell">{{ $r->created_at->format('d/m/Y H:i') }}</td><td class="table-cell font-semibold">{{ $r->student->name }}</td><td class="table-cell">{{ $r->violation->name }}</td><td class="table-cell"><span class="badge badge-red">{{ $r->violation->poin }}</span></td><td class="table-cell text-sm uppercase">{{ str_replace('_',' ',$r->violation->category) }}</td><td class="table-cell text-sm text-muted">{{ $r->note ?: '—' }}</td></tr>
                @empty
