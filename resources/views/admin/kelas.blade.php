...
<div class="flex justify-end mb-6 gap-2">
    <button onclick="confirmAction('Hapus SEMUA kelas yang tidak memiliki siswa?', () => document.getElementById('destroyAllKelas').submit())" class="btn-danger !text-sm"><i class="bi bi-trash"></i> Hapus Kelas Kosong</button>
    <form id="destroyAllKelas" action="{{ route('admin.kelas.destroy-all') }}" method="POST" class="hidden">@csrf @method('DELETE')</form>
    <button onclick="showModal('tambah')" class="btn-brand !text-sm"><i class="bi bi-plus-lg"></i> Tambah Kelas</button>
</div>
    ...
    <div class="table-container">
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr>
                    <td class="table-cell text-sm font-mono">{{ $classes->firstItem() + $i }}</td>
                    <td class="table-cell font-semibold">{{ $k->name }}</td>
                    <td class="table-cell">{{ $k->students_count }}</td>
                    <td class="table-cell-aksi">
                        <div class="flex items-center justify-center gap-1">
                            <button onclick="lihatKelas({{ $k->id }}, '{{ $k->name }}')" class="inline-flex items-center gap-1 px-2 py-1.5 rounded text-sm font-semibold border border-[#c3c6d1] text-[#43474f] hover:bg-[#edeeef] transition cursor-pointer"><i class="bi bi-eye"></i> Lihat</button>
                            <button onclick='editKelas({{ $k->id }}, "{{ $k->name }}")' class="inline-flex items-center gap-1 px-2 py-1.5 rounded text-sm font-semibold bg-blue-600 text-white hover:bg-blue-700 transition cursor-pointer"><i class="bi bi-pencil"></i> Ubah</button>
                            <button onclick="confirmAction('Hapus kelas {{ $k->name }}? Semua siswa akan kehilangan kelas.', () => deleteKelas({{ $k->id }}))" class="inline-flex items-center gap-1 px-2 py-1.5 rounded text-sm font-semibold bg-red-600 text-white hover:bg-red-700 transition cursor-pointer"><i class="bi bi-trash"></i> Hapus</button>
                        </div>
                    </td>
                </tr>
                @empty
...
function showModal(type, id, name) {
    ...
    box.innerHTML = `<div class="flex justify-between items-center mb-4"><h3 class="font-bold text-lg">${title}</h3><button onclick="closeModal()" class="text-slate-400 hover:text-slate-600"><i class="bi bi-x-lg text-xl"></i></button></div>
        <form action="${action}" method="POST" class="space-y-3" onsubmit="setLoading(this.querySelector('[type=submit]'), true)">
            @csrf ${method}
            <div><label class="text-sm font-bold text-slate-500 uppercase tracking-wider mb-1 block">Nama Kelas <span class="text-red-500">*</span></label><input type="text" name="name" class="input-field" value="${name||''}" placeholder="Contoh: XII IPA 1" required></div>
...
function lihatKelas(id, name) {
    ...
    box.innerHTML = '<div class="text-center py-8"><div class="w-8 h-8 border-4 border-[#1c6880] border-t-transparent rounded-full animate-spin mx-auto mb-3"></div><p class="text-sm text-muted">Memuat data...</p></div>';
...
function renderKelasBox(id, name, students) {
    const rows = students.length === 0
        ...
        : students.map((s, i) => `<tr>
            ...
            <td class="table-cell font-mono text-sm">${s.nis}</td>
            <td class="table-cell font-semibold">${s.name}</td>
            <td class="table-cell text-sm">${s.gender || '—'}</td>
            <td class="table-cell-aksi">
                <button onclick="lihatSiswa(${s.id})" class="inline-flex items-center gap-1 px-2 py-1.5 rounded text-sm font-semibold border border-[#c3c6d1] text-[#43474f] hover:bg-[#edeeef] transition cursor-pointer"><i class="bi bi-eye"></i> Lihat</button>
                <button onclick="confirmAction('Keluarkan ${s.name.replace(/'/g, "\\'")} dari kelas ini?', () => removeStudent(${id}, ${s.id}))" class="inline-flex items-center gap-1 px-2 py-1.5 rounded text-sm font-semibold bg-red-600 text-white hover:bg-red-700 transition cursor-pointer"><i class="bi bi-x-circle"></i> Keluarkan</button>
            </td>
        </tr>`).join('');

    document.getElementById('kelasBox').innerHTML = `
        <div class="flex justify-between items-start mb-4">
            <div><h3 class="font-bold text-lg">Kelas ${name}</h3><p class="text-sm text-muted">${students.length} siswa</p></div>
            <div class="flex gap-2">
                <button onclick="showAddStudent(${id})" class="btn-outline !text-sm"><i class="bi bi-plus-lg"></i> Tambah Siswa</button>
        ...
        <div id="searchAddBox_${id}" class="hidden mb-3 p-3 bg-slate-50 rounded-lg border border-[#c3c6d1]/30">
            <label class="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2 block">Cari Siswa (Nama / NIS)</label>
            <input type="text" id="searchInput_${id}" class="input-field !py-2" placeholder="Ketik minimal 2 karakter..." oninput="searchSiswa(${id}, this.value)">
...
function searchSiswa(classId, q) {
    ...
    results.innerHTML = '<p class="text-sm text-muted p-2">Mencari...</p>';
    searchTimer = setTimeout(() => {
        fetch('/admin/siswa/search?q=' + encodeURIComponent(q), { headers: { 'Accept': 'application/json' } })
            .then(r => r.json())
            .then(students => {
                if (students.length === 0) { results.innerHTML = '<p class="text-sm text-muted p-2">Tidak ditemukan.</p>'; return; }
                results.innerHTML = students.map(s => `<div class="flex items-center justify-between px-2 py-1.5 hover:bg-white rounded cursor-pointer border-b border-slate-100 last:border-0" onclick="assignStudent(${classId}, ${s.id}, '${s.name.replace(/'/g, "\\'")}')">
                    <div><span class="text-sm font-semibold">${s.name}</span><span class="text-xs text-muted ml-2">NIS: ${s.nis}</span></div>
                    <span class="text-xs text-blue-600 font-semibold"><i class="bi bi-plus-circle"></i> Tambah</span>
                </div>`).join('');
            });
    }, 300);
}
...
function lihatSiswa(id) {
    ...
    box.innerHTML = '<div class="text-center py-8"><div class="w-8 h-8 border-4 border-[#1c6880] border-t-transparent rounded-full animate-spin mx-auto mb-3"></div><p class="text-sm text-muted">Memuat...</p></div>';
    ...
    fetch('/admin/siswa/' + id + '/detail', { headers: { 'Accept': 'application/json' } })
        ...
        .then(s => {
            ...
            let html = `<div class="flex justify-between items-start mb-5"><div class="flex items-start gap-4"><div class="w-20 h-24 bg-slate-100 border-2 border-dashed border-slate-300 rounded flex items-center justify-center shrink-0"><div class="text-center"><i class="bi bi-person-fill text-3xl text-slate-300"></i><p class="text-[10px] text-slate-400 mt-0.5">3x4</p></div></div><div><h3 class="font-bold text-xl">${s.name}</h3><p class="text-sm font-mono text-muted mt-1"><strong>NIS:</strong> <strong class="text-[#1c6880]">${s.nis}</strong> &middot; <strong>NISN:</strong> <strong class="text-[#1c6880]">${s.nisn || '—'}</strong></p><p class="text-sm text-muted">${s.school_class?.name || '—'}</p></div></div><button onclick="closeDetail()" class="text-slate-400 hover:text-slate-600 shrink-0"><i class="bi bi-x-lg text-2xl"></i></button></div>`;
            html += `<div class="grid grid-cols-3 gap-4 text-sm mb-4">
                ...
                <div class="col-span-3"><span class="text-sm text-muted">Alamat</span><p class="font-semibold">${s.address || '—'}</p></div></div>`;
            ...
            if (father.name) html += `<div class="p-3 bg-slate-50 rounded-xl"><span class="text-sm text-muted">Ayah</span><p class="font-semibold">${father.name || '—'}</p><p class="text-sm text-muted">${father.job || ''} &middot; ${father.phone || ''}</p></div>`;
            if (mother.name) html += `<div class="p-3 bg-slate-50 rounded-xl"><span class="text-sm text-muted">Ibu</span><p class="font-semibold">${mother.name || '—'}</p><p class="text-sm text-muted">${mother.job || ''} &middot; ${mother.phone || ''}</p></div>`;
