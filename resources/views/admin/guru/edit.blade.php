    ...
    <form method="POST" action="{{ route('admin.guru.update', $guru) }}" class="space-y-6"
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div>
                ...
                <input id="name" type="text" name="nama" value="{{ old('nama', $guru->nama) }}" required
            ...
            <div>
                <label for="nip" class="block text-sm font-medium text-gray-700">NIP / NIK <span class="text-red-500">*</span></label>
                <input id="nip" type="text" name="nip" value="{{ old('nip', $guru->profilGuru?->nip) }}" required
                       class="mt-1.5 block w-full rounded-lg border border-gray-300 px-4 py-3 text-sm text-gray-900 shadow-sm
                              placeholder:text-gray-400 focus:border-primary focus:ring-2 focus:ring-primary/20
                              transition-colors"
            ...
            <div x-data="{ role: '{{ old('peran', $guru->peran) }}' }">
                <div>
                ...
                <div x-show="role === 'bk'" x-cloak class="mt-6">
                    ...
                    <select id="grade" name="tingkat"
                        ...
                        <option value="10" {{ old('tingkat', $guru->profilGuru?->tingkat) === '10' ? 'selected' : '' }}>10</option>
                        <option value="11" {{ old('tingkat', $guru->profilGuru?->tingkat) === '11' ? 'selected' : '' }}>11</option>
                        <option value="12" {{ old('tingkat', $guru->profilGuru?->tingkat) === '12' ? 'selected' : '' }}>12</option>
                    </select>
                    @error('tingkat')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div x-show="role === 'wali_kelas'" x-cloak class="mt-6">
                    ...
                    <select id="kelas_id" name="kelas_id"
                        ...
                        @foreach ($availableKelas as $kelas)
                            <option value="{{ $kelas->id }}" {{ old('kelas_id', $guru->kelasWali?->id) == $kelas->id ? 'selected' : '' }}>{{ $kelas->nama }}</option>
                        @endforeach
                    </select>
                    <p class="mt-1.5 text-xs text-gray-500">Cuma kelas yang belum ada wali kelasnya (atau kelas yang lagi dipegang guru ini) yang muncul di sini.</p>
                    @error('kelas_id')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
