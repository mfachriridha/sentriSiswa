...
<div class="rounded-xl border border-gray-200 bg-white p-6">
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <form method="GET" action="{{ route('bk.laporan.index') }}" class="grid gap-3 sm:grid-cols-2 lg:grid-cols-7">
            <div>
            ...
            <div>
                ...
                <select id="class_id" name="class_id" class="mt-1 w-full rounded-lg border border-gray-300 px-4 py-2 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20">
                    <option value="">Semua Kelas</option>
                    @foreach ($classes as $class)
                        <option value="{{ $class->id }}" {{ (string) $selectedClassId === (string) $class->id ? 'selected' : '' }}>{{ $class->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                @forelse($students as $index => $student)
                    @php $stat = $stats[$student->id]; @endphp
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="whitespace-nowrap px-4 py-3 text-sm text-gray-500">{{ $student->class?->name ?? '-' }}</td>
