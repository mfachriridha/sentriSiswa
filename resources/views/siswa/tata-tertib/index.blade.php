...
<div class="rounded-xl border border-gray-200 bg-white p-4 lg:p-6">
    @if ($schoolRule)
        <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-lg font-semibold text-gray-900">{{ $schoolRule->title }}</h2>
                <p class="mt-1 text-sm text-gray-500">Diperbarui {{ $schoolRule->updated_at->translatedFormat('d F Y') }}</p>
            </div>
            <a href="{{ asset('storage/'.$schoolRule->file_path) }}" target="_blank"
        ...
        <iframe src="{{ asset('storage/'.$schoolRule->file_path) }}" class="h-[70vh] w-full rounded-lg border border-gray-200"></iframe>
    @else
