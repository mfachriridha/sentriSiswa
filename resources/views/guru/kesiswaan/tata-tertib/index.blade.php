...
<div class="mb-5 rounded-xl border border-gray-200 bg-white p-4 lg:p-6">
    <form method="POST" action="{{ route('guru.kesiswaan.tata-tertib.store') }}" enctype="multipart/form-data" class="grid gap-4 lg:grid-cols-4">
        @csrf
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($schoolRules as $rule)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-gray-600">{{ $rule->uploadedBy?->name ?? '-' }} · {{ $rule->created_at->translatedFormat('d M Y') }}</td>
                        <td class="px-4 py-3">
                            <div class="flex justify-end gap-2">
                                @if ($rule->is_published)
                                    <form method="POST" action="{{ route('guru.kesiswaan.tata-tertib.unpublish', $rule) }}">
                                        @csrf
                                        @method('PUT')
                                ...
                                @else
                                    <form method="POST" action="{{ route('guru.kesiswaan.tata-tertib.publish', $rule) }}">
                                        @csrf
                                        @method('PUT')
                                ...
                                @endif
                                <form method="POST" action="{{ route('guru.kesiswaan.tata-tertib.destroy', $rule) }}">
                                    @csrf
                                    @method('DELETE')
