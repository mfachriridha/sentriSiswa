...
<div class="rounded-xl border border-gray-200 bg-white p-8">
    <h1 class="mb-8 text-2xl font-bold text-gray-900">Edit Pelanggaran Siswa</h1>

    <form method="POST" action="{{ route('guru.pelanggaran-siswa.update', $studentViolation) }}" class="space-y-6" x-data="{ loading: false }" @submit="loading = true">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="notes" name="notes" rows="4"
                          class="mt-1.5 block w-full rounded-lg border border-gray-300 px-4 py-3 text-base text-gray-900 shadow-sm placeholder:text-gray-400 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors">{{ old('notes', $studentViolation->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
